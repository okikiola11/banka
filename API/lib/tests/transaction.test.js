...
var clientToken = _jsonwebtoken.default.sign({
  id: 3456,
  type: 'client',
  isAdmin: false
}, process.env.SECRET, {
...
describe('/ Transaction Account Endpoint ', function () {
  describe('/ POST credit account transaction', function () {
    ...
    it('should restrict non-staff members (i.e clients) from crediting an account', function (done) {
      (0, _supertest.default)(_index.default).post("".concat(API_PREFIX, "/transactions/2050030401/credit")).set('Accept', 'application/json').set('Authorization', "".concat(clientToken)).send({
        amount: '25000.00'
...
describe('/ Transaction Account Endpoint ', function () {
  describe('/ POST debit account transaction', function () {
    ...
    it('should restrict non-staff members (i.e clients) from debiting an account', function (done) {
      (0, _supertest.default)(_index.default).post("".concat(API_PREFIX, "/transactions/2050030455/debit")).set('Accept', 'application/json').set('Authorization', "".concat(clientToken)).send({
        amount: '25000.00'
      }).expect(403).expect(function (response) {
        expect(response.body).to.have.all.keys('status', 'message');
        expect(response.body.status).to.equal(403);
